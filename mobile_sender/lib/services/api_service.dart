...
class ApiService {
  ...
  Future<Map<String, dynamic>> calculatePrice(Map<String, dynamic> priceData) async {
    final response = await http.post(
      Uri.parse('${AppConfig.apiBaseUrl}${AppConfig.bookingEndpoint}/calculate-price'),
      headers: _headers,
  ...
  // Parcels Endpoints
  Future<List<Parcel>> getParcels({String? status, int? limit, int? offset}) async {
  ...
  Future<Parcel> getParcelDetails(String parcelId) async {
    final response = await http.get(
      Uri.parse('${AppConfig.apiBaseUrl}${AppConfig.parcelsEndpoint}/$parcelId'),
      headers: _headers,
    );
  ...
  Future<Parcel> trackParcel(String parcelNumber) async {
    final response = await http.get(
      Uri.parse('${AppConfig.apiBaseUrl}${AppConfig.parcelsEndpoint}/track/$parcelNumber'),
      headers: _headers,
    );
  ...
  Future<Map<String, dynamic>> cancelParcel(String parcelId, String reason) async {
    final response = await http.post(
      Uri.parse('${AppConfig.apiBaseUrl}${AppConfig.parcelsEndpoint}/$parcelId/cancel'),
      headers: _headers,
  ...
  Future<Map<String, dynamic>> rateDelivery(String parcelId, int rating, String? feedback) async {
    final response = await http.post(
      Uri.parse('${AppConfig.apiBaseUrl}${AppConfig.parcelsEndpoint}/$parcelId/rate'),
      headers: _headers,
      body: json.encode({
        'rating': rating,
        'feedback': feedback,
